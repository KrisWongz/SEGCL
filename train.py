...
from models import LogReg, SE_GCL
...
def run(args, gpu_id=None, **kwargs):
    # ===================================================#
    ...
    cprint("## Done ##", "yellow")
    # ===================================================#
    model = SE_GCL(nb_feature, cfg=args.cfg,
    ...
    for current_iter, epoch in enumerate(tqdm(range(args.start_epoch, args.start_epoch + args.epochs + 1))):
        ...
        if epoch % args.epochs == 0 and epoch != 0:
            ...
            if args.custom_key == 'classification':
                if args.dataset_name == 'WikiCS':
...
if __name__ == '__main__':
    ...
    run(main_args, gpu_id=alloc_gpu[0])
